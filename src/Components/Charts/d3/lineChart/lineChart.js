...
export default function LineChart(config) {
  ...
  var reactW =
    textSize *
    (tipTitle[0].length > tipTitle[1].length
      ? tipTitle[0].length
      : tipTitle[1].length);
  reactW += 10; //预设10留白
  ...
  var svgG = svg
    ...
    .attr(
      'transform',
      'translate(' + Number(margin[2] + 40) + ',' + margin[0] + ')',
    );
  var parseDate = d3.timeParse('%Y-%m-%d %H:%M:%S');
  var formatDate = d3.timeFormat('%Y-%m-%d %H:%M:%S');

  //计算比例尺
  var xRange = (xRange = d3.extent(DataSet, function(d) {
  ...
  var xAxis = svgG
    .append('g')
    .attr('class', 'xAxis')
    .attr('transform', 'translate(0,' + Number(H) + ')');
  yAxis.call(
    d3
      .axisLeft(y)
      .ticks(10)
  ...
  function move() {
    ...
    var d =
      mouseDate - parseDate(d0.date) > parseDate(d1.date) - mouseDate ? d1 : d0;
    var d =
      mouseDate - parseDate(d0.date) > parseDate(d1.date) - mouseDate ? d1 : d0;
    tipData = i;
    ...
    MarkTip0.style('display', 'inline');
    if (mouse1[1] > (H + 30) / 2) {
      MarkTip0.select('g.rotate')
        .attr('transform-origin', '0,0')
        .attr('transform', 'rotate(0)');

      if (mouse1[0] < W / 2) {
        MarkTip0.select('g.translate').attr(
          'transform',
          'translate(20,' + (rectH + 7) + ')',
        );
        MarkTip0.attr('x', mouse1[0] - 25).attr('y', mouse1[1] - (rectH + 18));
      } else {
        MarkTip0.select('g.translate').attr(
          'transform',
          'translate(' + (reactW - 15) + ',' + (rectH + 8) + ')',
        );
        MarkTip0.attr('x', mouse1[0] - reactW + 10).attr(
          'y',
          mouse1[1] - (rectH + 19),
        );
      }
      MarkTip0.append('image')
        .attr('link:href', dTipIcon[0])
        .attr('width', textSize * 0.9)
        .attr('x', 10)
        .attr('y', 5 + 2.65 * textSize);
      MarkTip0.append('image')
        .attr('link:href', dTipIcon[1])
        .attr('width', textSize * 0.9)
        .attr('x', 10)
        .attr('y', 5 + 4.05 * textSize);
      MarkTip0.append('text')
        .style('font-size', textSize * 0.8 + 'px')
        .attr('x', 10)
        .attr('y', 5 + 2.0 * textSize)
        .text(d.date);
      MarkTip0.append('text')
        .style('font-size', textSize * 0.9 + 'px')
        .style('font-weight', 600)
        .attr('x', 10 + textSize)
        .attr('y', 5 + 3.4 * textSize)
        .text('累计销售额' + value);
      MarkTip0.append('text')
        .style('font-size', textSize * 0.9 + 'px')
        .style('font-weight', 600)
        .attr('x', 10 + textSize)
        .attr('y', 5 + 4.8 * textSize)
      ...
      if (mouse1[0] < W / 2) {
        ...
        MarkTip0.attr('x', mouse1[0] - 15).attr('y', mouse1[1]);
      } else {
        MarkTip0.select('g.translate').attr(
          'transform',
          'translate(' + (reactW - 10) + ',12)',
        );
        MarkTip0.attr('x', mouse1[0] - reactW + 15).attr('y', mouse1[1]);
      }
      MarkTip0.append('image')
        .attr('link:href', dTipIcon[0])
        .attr('width', textSize * 0.9)
  ...
  this.addData = function(data) {
    DataSet = data;
    //更新比例尺
    xRange = xRange = d3.extent(DataSet, function(d) {
      return parseDate(d.date);
    });
    x = d3
      .scaleLinear()
      ...
      .on('mousemove.2', function(d, i) {
        ...
        if (mouse[1] > (H + 30) / 2) {
          ...
          if (mouse[0] < W / 2) {
            MarkTip0.select('g.translate').attr(
              'transform',
              'translate(20,' + (rectH + 8) + ')',
            );
            MarkTip0.attr('x', mouse[0] - 25).attr(
              'y',
              mouse[1] - (rectH + 18),
            );
          } else {
            MarkTip0.select('g.translate').attr(
              'transform',
              'translate(' + (reactW - 20) + ',' + (rectH + 8) + ')',
            );
            MarkTip0.attr('x', mouse[0] - reactW + 20).attr(
              'y',
              mouse[1] - (rectH + 18),
            );
          }
          ...
          if (mouse[0] < W / 2) {
            ...
            MarkTip0.attr('x', mouse[0] - 15).attr('y', mouse[1]);
          } else {
            MarkTip0.select('g.translate').attr(
              'transform',
              'translate(' + (reactW - 20) + ',12)',
            );
            MarkTip0.attr('x', mouse[0] - reactW + 25).attr('y', mouse[1]);
          }
        }
        var value = (d.value / digit).toFixed(2) + yUint;
        MarkTip0.append('text')
  ...
  MarkTip0.append('g')
    ...
    .attr(
      'd',
      'M512 844.8L759.466667 512l247.466666-332.8H17.066667L264.533333 512z',
    );
  MarkTip0.append('g')
